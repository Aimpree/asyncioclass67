...
async def main():
    #coffee_task = asyncio.create_task(PourCoffee())
    #coffee = await coffee_task
    # print(coffee)

    PourCoffee()
    print(f"{time.ctime()} - >>>>> Coffee is being prepared\n")

   
    task1 = asyncio.create_task(FryEggs(2))
    task2 = asyncio.create_task(FryBacon())
    task3 = asyncio.create_task(ToastBread(2))
    ...
    PourJuice()
    #juice_task = asyncio.create_task(PourJuice())
    #juice = await juice_task
    # print(juice)

    print(f"{time.ctime()} - Breakfast is ready with coffee and juice.")
